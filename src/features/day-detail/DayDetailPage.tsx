...
import { getDayRecord, saveJournal, savePhoto, setDayStatusFromRequirements, setTask } from "../../storage/repository";
...
export function DayDetailPage({ state, onChange }: { state: ActiveChallengeState; onChange: () => Promise<void> }) {
  ...
  const saveChanges = async () => {
    ...
    if (pendingPhotoFile) {
      await savePhoto(day.id, pendingPhotoFile, { preserveDayStatus: true });
    }
    ...
    <main className="space-y-8 px-5 py-8">
      ...
      {editMode && editable ? (
        <DayEditor
          ...
          onPhotoSelect={(file) => void selectPhoto(file, setPendingPhotoFile, setPendingPhotoUrl, setTasks)}
...
async function selectPhoto(file: File | undefined, setPendingPhotoFile: Dispatch<SetStateAction<File | undefined>>, setPendingPhotoUrl: Dispatch<SetStateAction<string | undefined>>, setTasks: Dispatch<SetStateAction<TaskCompletion[]>>) {
  if (!file) return;
